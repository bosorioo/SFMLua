...
{
    LuaAux::newClass(L, LuaGif::Methods, LuaGif::Meta, GIF_TABLE, LuaGif::Call);
}

void LuaGif::setGifError(std::string& out, int error)
{
    switch (error)
    {
...
bool LuaGif::loadGif(GifImage& gif, const char* path, std::string& error_out)
{
    int error;
    ...
    if (!gif.canvas.create(gifFile->SWidth, gifFile->SHeight, false))
    {
        DGifCloseFile(gifFile, NULL);
        return false;
    }
...
void LuaGif::unloadGif(GifImage& gif)
{
...
int LuaGif::Call(lua_State* L)
{
    lua_remove(L, 1);
    return LuaGif::Create(L);
}

int LuaGif::Create(lua_State* L)
{
...
int LuaGif::LoadFromFile(lua_State* L)
{
...
int LuaGif::GetSprite(lua_State* L)
{
    GifImage* gif = LuaAux::getUserData<GifImage>(L, 1, GIF_TABLE);
...
int LuaGif::GetCanvas(lua_State* L)
{
    GifImage* gif = LuaAux::getUserData<GifImage>(L, 1, GIF_TABLE);
...
int LuaGif::GetFrameCount(lua_State* L)
{
    GifImage* gif = LuaAux::getUserData<GifImage>(L, 1, GIF_TABLE);
...
int LuaGif::SetFrameIndex(lua_State* L)
{
    GifImage* gif = LuaAux::getUserData<GifImage>(L, 1, GIF_TABLE);
...
int LuaGif::StepFrame(lua_State* L)
{
    GifImage* gif = LuaAux::getUserData<GifImage>(L, 1, GIF_TABLE);
...
int LuaGif::Destroy (lua_State* L)
{
    SFMLuaClassDestroy(GifImage);
    return 0;
}

LuaGif::GifImage::~GifImage()
{
    unloadGif(*this);
}
